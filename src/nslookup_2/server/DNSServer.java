        ...
        public void run() {
            ...
            try (BufferedReader in = new BufferedReader(new InputStreamReader(socket.getInputStream()));
                ...
                if (domain != null && !domain.trim().isEmpty()) {
                    System.out.printf("[%s] Client #%d querying: %s\n", timestamp, clientId, domain);
                    ...
                    out.println("END");
                    System.out.printf("[%s] Client #%d completed in %dms\n",
                            timestamp, clientId, (endTime - startTime));
                } else {
                    out.println("Error: Empty query");
                    out.println("END");
                }
            } catch (IOException e) {
                System.err.printf("[%s] Client #%d error: %s\n", timestamp, clientId, e.getMessage());
            } finally {
                try {
                    socket.close();
                } catch (IOException e) {
